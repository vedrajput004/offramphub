...
document.querySelectorAll('.team-inner').forEach(block => {
    ...
    timeline
        ...
        .to(hoverImage, { opacity: 1, y: 0, scale: 1, duration: 0.5, position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", ease: "power2.inOut" })
        .to(leftLogo, { x: "0%", opacity: 1, duration: 0.5, ease: "power2.out" })
        .to(rightLogo, { x: "0%", opacity: 1, duration: 0.5, ease: "power2.out" }, "-=0.5")
        .to(hoverImage, { scale: 0.3, duration: 0.5, position: "absolute", top: 0, left: 0, transform: "translate(0,0)", ease: "power2.inOut" })

        .fromTo(detailText, { opacity: 0, y: 50 }, { opacity: 1, y: 0, duration: 0.5, ease: "power2.out" });
    ...
    block.addEventListener('mouseleave', () => {
        gsap.to(hoverImage, { opacity: 0, y: 50, duration: 0.7, ease: "power2.inOut" });
        gsap.to([leftLogo, rightLogo], { x: ["-100%", "100%"], opacity: 0, duration: 0.5, ease: "power2.in" });
        gsap.fromTo(detailText, { opacity: 1, y: 0 }, { opacity: 0, y: 50, duration: 0.5, ease: "power2.out" });
    });
});
